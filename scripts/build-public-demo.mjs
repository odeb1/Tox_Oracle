...
for(const name of ['public.css','public.js','public-model.js'])await cp(path.join(pub,name),path.join(out,'static',name));
...
const expected=new Set(['index.html','vercel.json',...['workspace.css','results-view.js','pose-viewer.js','public.css','public.js','public-model.js'].map(n=>'static/'+n),'studies/index.json']);
